...
module DataMemory(Address, WriteData, Clk, MemWrite, MemRead, ReadData); 
    ...
    // 1K x 32-bit memory array (indices 0..1023)
    reg [31:0] memory [0:1023];

    // Word index derived from byte address: use Address[11:2]
    wire [9:0] word_index = Address[11:2];

    // Asynchronous read: drive ReadData with memory[word_index] when MemRead=1,
    // else drive zero.
    always @(*) begin
        if (MemRead)
